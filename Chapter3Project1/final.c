...
void clearscreen()
{
    ...
    if (first)
    {
        const *CLEAR_SCREEN_ANSI = "\e[1;1H\e[2J";
    ...
    { // child process
        ...
        { // pipe
            ...
            { // child process for the second command
                ...
                char *input, *out;
                int input_fd, out_fd;
                unsigned flag = ch_redirect(args2, &args_num2, &input, &out); // bit 1 for output, bit 0 for input
            ...
            { // grandchild process for the first command
                ...
                char *input, *out;
                int input_fd, out_fd;
                unsigned flag = ch_redirect(args, &args_num, &input, &out); // bit 1 for output, bit 0 for input
